...
contract TokenVault {
    ...
    function emergencyRecovery(address payable recipient) external onlyOwner {
        require(emergencyLock, "Emergency not active");
        uint256 vaultBalance = address(this).balance;
        require(vaultBalance > 0, "No funds to recover");

        (bool success, ) = recipient.call{value: vaultBalance}("");
        require(success, "Recovery transfer failed");
        emit EmergencyRecovery(recipient, vaultBalance);
    }
